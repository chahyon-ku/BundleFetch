...
class Frame(object):
    def __init__(self, frame_stop, track_queue, frame_type, spot_queue) -> None:
    ...
    def __call__(self):
        ...
        print('[FRAME] frame process started')
        if self.source is None:
            if self.frame_type == 'realsense':
                self.source = FrameRealsense()
            elif self.frame_type == 'spot':
                self.source = FrameSpot()
            elif self.frame_type == 'dataset':
                self.source = FrameDataset('/media/rpm/Data/imitation_learning/BundleFetch/data/test_multiobj')
            else:
                raise NotImplementedError
        i_frame = 0
        while not self.frame_stop.is_set():
            # print('[FRAME] frame', i_frame)
            frame = self.source.get_frame(self.spot_queue)
            if frame is None:
                break
...
def process_frame(frame, uv1):
    ...
    frame['xyz'] = frame['xyz'].reshape(3, W, H).permute(0, 2, 1) # (3, H, W)
